...
public class MovieInfo extends AppCompatActivity implements MovieAdapter.onClickListener, MovieAdapter.onClickListen {
    ...
    String movieIMDBRating, userId, trailerId, movieCategory, movieImdbId;
    int counter = 0;
    ImageView imageView;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        genreList = Arrays.asList(movieGenre.split(","));
        ...
        thread1 = new Thread(new Runnable() {
            ...
            public void run() {
                ...
                for (int num = 1; num <= 30000; num++) {
                    movieCategory = dataArr[num][1];
                    if (movieCategory.toLowerCase().contains(movieGenre.toLowerCase())) {
                        counter++;
    ...
    public void rateHere(View view) {
    ...
    public void getUri(String trailerId) {
        watchTrailer.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent intent = new Intent(MovieInfo.this, TrailerScreen.class);
                intent.putExtra("Url", trailerId);
                Log.d(TAG, "onClick: " + trailerId);
                startActivity(intent);
            }
        });
    }

    private void getVideoUrl(String movieId) {
        requestQueue = Volley.newRequestQueue(this);
        jsonObjectRequest = new JsonObjectRequest(Request.Method.GET, "https://api.themoviedb.org/3/movie/" + movieId + "/videos?api_key=f8a9c403a350feed14529b21c1363e2a", null, new Response.Listener<JSONObject>() {
            @Override
            public void onResponse(JSONObject response) {
                try {
                    JSONArray array = response.getJSONArray("results");
                    Log.d(TAG, "onResponse: Length of results " + array.length());
                    for (int i = 0; i < array.length(); i++) {
                        JSONObject object = array.getJSONObject(0);
                        trailerId = object.getString("key");
                    }
    ...
    private void getRecommendedMovies(String movieId) {
        requestQueue = Volley.newRequestQueue(this);
        jsonObjectRequest = new JsonObjectRequest(Request.Method.GET, "https://recommendsome.herokuapp.com/movie/" + movieId, null, new Response.Listener<JSONObject>() {
            @Override
            public void onResponse(JSONObject response) {
                try {
                    Log.d(TAG, "onResponse: https://recommendsome.herokuapp.com/movie/" + movieId);
                    //int a = 5/0;
                    JSONArray array = response.getJSONArray("recommendations");
                    for (int i = 0; i < array.length(); i++) {
                        JSONArray first = array.getJSONArray(i);
                        getData(first.getString(3), first.getString(4), first.getString(5));
                    }

                } catch (Exception e) {
                    Log.d(TAG, "onResponse: Some Error");
                }
            }
        }, new Response.ErrorListener() {
            ...
            @Override
            public void onErrorResponse(VolleyError error) {
                String[] array = new String[genreList.size()];

                for (int i = 0; i < genreList.size(); i++) {
                    if (genreList.get(i).equals(" Sci-Fi"))
                        genreList.set(i, "Science Fiction");
                    array[i] = genreList.get(i).trim();
                }

                for (int num = 1; num <= 30000; num++) {
                    movieCategory = dataArr[num][4];
                    //  Log.d(TAG, "onErrorResponse: "+movieCategory);
                    if (Arrays.stream(array).allMatch(movieCategory::contains)) {

                        Log.d(TAG, "onErrorResponse: " + num);
                        counter++;
                        movieImdbId = dataArr[num][0];
                        movieTagline = dataArr[num][2];
                        movieOverview = dataArr[num][3];
                        getData(movieImdbId, movieTagline, movieOverview);
                        if (counter == 20)
                            break;
                    }
                }
    ...
    private void getData(String url, String tagline, String overview) {
        ...
        jsonObjectRequest = new JsonObjectRequest(Request.Method.GET, "https://www.omdbapi.com/?i=" + url + "&apikey=cfdb5a43", null, new Response.Listener<JSONObject>() {
            ...
            public void onResponse(JSONObject response) {
                try {
                    //  Log.d(TAG, "onResponse: Get data from omdb"+url);
